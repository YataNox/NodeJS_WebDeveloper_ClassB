...
// 다만 리턴되는 cookie에 split, map, reduce 등등의 관련 객체멤버함수가 실행될 예정입니다.
// const parseCookies = (cookie='')=>cookie.split().map().reduce();
const parseCookies = (cookie='')=>
    cookie
        .split(';')  //cookie1=test1;cookie2=test2 ';'으로 구분되어 전달된 쿠키를 분리합니다
        .map(v=>v.split('=') )   // 분리된 쿠키들이 v에 전달되어 그들을 다시 '='로 분리됩니다. v에 분리된 쿠키가 저장됩니다 
        // 그렇게 분리된 둘은 k와 v에 전달되어 객체 형태를 이루고 acc 저장되어 최종 객체 형태의 데이터로 리턴됩니다
        .reduce( (acc, [k, v])=>{
...
http.createServer( async(req, res)=>{
    ...
    // 주소가 /login으로 시작하는 경우
        ...
        // 경로 및 위치 / , Expires 유효시간, HttpOnly 쿠키의 접근을 http방식으로만으로 제한
        res.end();

    }else if(cookies.name){ 
    // if문에 있는 req.url이 /login으로 시작하지 않은 경우중, name쿠키가 있는 경우 
